...
Deduplication
-------------
Each fixture is uploaded to an Onshape document whose name is
``g4occt/<fixture_id>`` (e.g. ``g4occt/direct-primitives/G4Box/box-20x30x40-v1``).
Before creating a new document the script searches the authenticated user's
documents for an existing one with the same name and reuses it if found.  This
avoids re-uploading files on every CI run.
...
def _discover_fixtures(manifest_path: Path) -> list[tuple[str, Path]]:
    ...
    NIST CTC and other fixtures whose STEP file has not yet been downloaded are
    silently skipped (the CI fetch step downloads them before this script runs,
    but the script is also called in contexts where they may be absent).
    """
    manifest = yaml.safe_load(manifest_path.read_text(encoding="utf-8"))
    fixture_root = Path(manifest["fixture_root"])
    if not fixture_root.is_absolute():
        # fixture_root is a path relative to the repository root.  The script is
        # always invoked from the repository root in CI, so resolve against CWD.
        fixture_root = Path.cwd() / fixture_root
    step_name = manifest.get("policy", {}).get("step_file_name", "shape.step")
    ...
    for family in manifest.get("families", []):
        ...
        if not family_dir.is_dir():
            continue
        for cls_dir in sorted(family_dir.iterdir()):
            if not cls_dir.is_dir():
                continue
            for fixture_dir in sorted(cls_dir.iterdir()):
                if not fixture_dir.is_dir():
                    continue
                step_path = fixture_dir / step_name
                if not step_path.is_file():
                    continue
                fixture_id = f"{family}/{cls_dir.name}/{fixture_dir.name}"
...
def _doc_name(fixture_id: str) -> str:
    """Return the canonical Onshape document name for a fixture."""
    return f"g4occt/{fixture_id}"
