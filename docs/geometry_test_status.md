...
| `twisted-swept` | 6 | 1 | 5 | `G4ExtrudedSolid` enforced; twisted surrogates remain approximate |
...
**Why they fail:**
Geant4's twisted solids have faces that follow a continuous helical or quadric
twist described analytically. OCCT has no native representation for such
surfaces. The STEP fixtures in the `twisted-swept` family are generated by an
OCCT ruled-loft utility that approximates the twisted shape with a sequence of
ruled surface patches. The resulting BRep differs geometrically from the
analytic Geant4 definition, so ray-intersection distances and inside/outside
classifications diverge at the boundary.

**Important context:** G4OCCT's primary goal is to import and navigate
arbitrary STEP geometry, not to provide a round-trip STEP export path for every
Geant4 primitive. The lack of an exact STEP representation for twisted
surfaces is therefore not, by itself, a project defect that must be closed for
G4OCCT to be useful.

**Work required to pass (optional):**
If a use case arises that requires strict parity for twisted fixtures, the path
forward would be:
...
| Improve twisted-solid STEP construction or explicitly retain surrogate xfails | `G4TwistedBox`, `G4TwistedTrap`, `G4TwistedTrd`, `G4TwistedTubs`, `G4VTwistedFaceted` | **Medium** |
