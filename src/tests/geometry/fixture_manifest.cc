...
std::pair<std::string, std::string> ParseKeyValue(
  ...
  if (separator == std::string::npos) {
    throw std::runtime_error(
        "Expected key/value pair in " + manifest_path.string() + ":" +
...
std::vector<std::string> ParseScalarList(
  ...
  while (index < lines.size()) {
    ...
    if (line.indent != parent_indent + 2 || line.content.rfind("- ", 0) != 0) {
      throw std::runtime_error(
          "Expected list item in " + manifest_path.string() + ":" +
          std::to_string(line.number));
    }
...
double ParseDouble(const std::string& text, const ParsedLine& line, const std::filesystem::path& path) {
  try {
    return std::stod(text);
  } catch (const std::exception&) {
    throw std::runtime_error(
        "Expected numeric value in " + path.string() + ":" + std::to_string(line.number));
  }
}

void ApplyFixtureField(
  ...
  if (key == "id") {
    ...
    throw std::runtime_error(
        "Unsupported fixture field '" + key + "' in " + manifest_path.string() + ":" +
        std::to_string(line.number));
  }
}

void ApplyExpectationField(
  ...
  if (key == "quantity") {
    ...
    throw std::runtime_error(
        "Unsupported expectation field '" + key + "' in " + manifest_path.string() + ":" +
        std::to_string(line.number));
  }
}

std::vector<FixtureExpectation> ParseExpectations(
    const std::vector<ParsedLine>& lines,
    std::size_t& index,
    const int parent_indent,
    const std::filesystem::path& manifest_path) {
  ...
  while (index < lines.size()) {
    ...
    if (line.indent != parent_indent + 2 || line.content.rfind("-", 0) != 0) {
      throw std::runtime_error(
          "Expected expectation list item in " + manifest_path.string() + ":" +
          std::to_string(line.number));
    }
    ...
    if (!rest.empty()) {
      const auto [key, value] = ParseKeyValue(ParsedLine{line.number, line.indent, rest}, manifest_path);
    ...
    while (index < lines.size()) {
      ...
      if (nested.indent != parent_indent + 4) {
        throw std::runtime_error(
            "Unexpected indentation for expectation field in " + manifest_path.string() + ":" +
            std::to_string(nested.number));
      }
...
std::vector<FixtureReference> ParseFixtures(
    const std::vector<ParsedLine>& lines,
    std::size_t& index,
    const int parent_indent,
    const std::filesystem::path& manifest_path) {
  ...
  while (index < lines.size()) {
    ...
    if (line.indent != parent_indent + 2 || line.content.rfind("-", 0) != 0) {
      throw std::runtime_error(
          "Expected fixture list item in " + manifest_path.string() + ":" +
          std::to_string(line.number));
    }
    ...
    while (index < lines.size()) {
      ...
      if (nested.indent != parent_indent + 4) {
        throw std::runtime_error(
            "Unexpected indentation for fixture field in " + manifest_path.string() + ":" +
            std::to_string(nested.number));
      }
      ...
      if (key == "expectations") {
        if (value.empty()) {
          ...
          throw std::runtime_error(
              "Expectations must be a block list in " + manifest_path.string() + ":" +
              std::to_string(nested.number));
        }
        continue;
      }
...
void AddCoverageClass(FixtureManifest& manifest, std::string class_name) {
  const auto existing = std::find(
      manifest.coverage_classes.begin(), manifest.coverage_classes.end(), class_name);
...
FixtureRepositoryManifest ParseFixtureRepositoryManifest(const std::filesystem::path& manifest_path) {
  ...
  for (std::size_t index = 0; index < lines.size();) {
    ...
    if (line.indent != 0) {
      throw std::runtime_error(
          "Unexpected indentation in " + manifest_path.string() + ":" + std::to_string(line.number));
    }
    ...
    if (key == "schema_version") {
      ...
      while (index < lines.size() && lines[index].indent > line.indent) {
        ...
        if (nested.indent != line.indent + 2) {
          throw std::runtime_error(
              "Unexpected indentation in policy block at " + manifest_path.string() + ":" +
              std::to_string(nested.number));
        }
        ...
        if (policy_key == "step_file_name") {
          ...
          throw std::runtime_error(
              "Unsupported policy field '" + policy_key + "' in " + manifest_path.string() + ":" +
              std::to_string(nested.number));
        }
        ++index;
      }
      ...
      if (!value.empty() && value != "[]") {
        throw std::runtime_error(
            "Families must be encoded as a block list in " + manifest_path.string() + ":" +
            std::to_string(line.number));
      }
      manifest.families = ParseScalarList(lines, index, line.indent, manifest_path);
    } else {
      throw std::runtime_error(
          "Unsupported repository manifest field '" + key + "' in " + manifest_path.string() + ":" +
          std::to_string(line.number));
    }
  }
  ...
  for (std::size_t index = 0; index < lines.size();) {
    ...
    if (key == "schema_version") {
      ...
      if (value.empty()) {
        ...
        throw std::runtime_error(
            "Fixtures must be encoded as a block list in " + manifest_path.string() + ":" +
            std::to_string(line.number));
      }
    } else {
      throw std::runtime_error(
          "Unsupported family manifest field '" + key + "' in " + manifest_path.string() + ":" +
          std::to_string(line.number));
    }
  }
