  ...
  while (std::getline(input, raw_line)) {
    ...
    if (!trimmed.empty()) {
      const bool is_continuation =
          !lines.empty() && indent > lines.back().indent && trimmed.rfind("- ", 0) != 0 &&
          !LooksLikeKeyValue(trimmed);
      if (is_continuation) {
...
std::pair<std::string, std::string> ParseKeyValue(
  ...
  if (separator == std::string::npos) {
    throw std::runtime_error(
        "Expected key/value pair in " + path.string() + ":" + std::to_string(line.number));
  }

  return {Trim(line.content.substr(0, separator)),
          Trim(line.content.substr(separator + 1))};
}
  ...
  while (index < lines.size()) {
    ...
    if (value.empty()) {
      if (index >= lines.size() || lines[index].indent <= indent) {
        throw std::runtime_error(
            "Expected nested block for key '" + key + "' in " + path.string() + ":" +
...
YamlNode ParseSequence(
  ...
  while (index < lines.size()) {
    ...
    if (rest.empty()) {
      ...
      if (index >= lines.size() || lines[index].indent <= indent) {
        throw std::runtime_error(
            "Expected nested list entry in " + path.string() + ":" + std::to_string(line.number));
      }
    ...
    if (!IsInlineList(rest) && LooksLikeKeyValue(rest)) {
      ...
      if (value.empty()) {
        if (index >= lines.size() || lines[index].indent <= indent + 2) {
          throw std::runtime_error(
              "Expected nested mapping value for key '" + key + "' in " + path.string() + ":" +
              std::to_string(line.number));
        }
      ...
      while (index < lines.size()) {
        ...
        if (nested_value.empty()) {
          if (index >= lines.size() || lines[index].indent <= indent + 2) {
            throw std::runtime_error(
                "Expected nested block for key '" + nested_key + "' in " + path.string() + ":" +
