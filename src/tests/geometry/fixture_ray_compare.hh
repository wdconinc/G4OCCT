 ...
 * - `G4Tet` and `G4TessellatedSolid`: centroid of all vertices from the
 *   fixture provenance (`vertices_mm`).  The vertex centroid lies inside
 *   any convex tessellated solid, guaranteeing `DistanceToOut` rays are
 *   used and every direction produces a hit.
