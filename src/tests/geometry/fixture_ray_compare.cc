...
const YamlNode& RequireNode(const YamlNode& parent, const std::string& key, const std::string& context) {
...
double ParseDouble(const YamlNode& node, const std::string& context) {
  try {
    ...
    throw std::runtime_error("Expected numeric YAML scalar in " + context + ": '" + node.AsScalar() + "'");
  }
}

std::string RequireString(const YamlNode& parent, const std::string& key, const std::string& context) {
...
std::vector<double> RequireDoubleList(const YamlNode& parent, const std::string& key, const std::string& context) {
  const auto& sequence = RequireNode(parent, key, context).AsSequence();
  std::vector<double> values;
  values.reserve(sequence.size());
  for (std::size_t index = 0; index < sequence.size(); ++index) {
    values.push_back(ParseDouble(sequence[index], context + "." + key + "[" + std::to_string(index) + "]"));
  }
  return values;
}

std::vector<G4ThreeVector> RequirePointList(
  ...
  for (std::size_t index = 0; index < sequence.size(); ++index) {
    ...
    if (coords.size() != 3U) {
      throw std::runtime_error(
          "Expected 3-vector YAML entry in " + context + "." + key + "[" + std::to_string(index) + "]");
    }
    points.emplace_back(ParseDouble(coords[0], context), ParseDouble(coords[1], context), ParseDouble(coords[2], context));
  }
  return points;
}

std::vector<G4TwoVector> RequirePlanarPointList(
    const YamlNode& parent,
    const std::string& key,
    const std::string& context) {
  const auto& sequence = RequireNode(parent, key, context).AsSequence();
  ...
  for (std::size_t index = 0; index < sequence.size(); ++index) {
    ...
    if (coords.size() != 2U) {
      throw std::runtime_error(
          "Expected 2-vector YAML entry in " + context + "." + key + "[" + std::to_string(index) + "]");
    }
...
G4ThreeVector RequireVector3(const YamlNode& parent, const std::string& key, const std::string& context) {
...
std::string Geant4Class(const FixtureProvenance& provenance) {
  return RequireString(ShapeNode(provenance), "geant4_class", provenance.source_path.string() + ".shape");
}

std::unique_ptr<G4VSolid> BuildBooleanBoxSolid(
    const std::string& geant4_class,
    const std::string& name,
    const YamlNode& shape,
    const std::string& context) {
  if (geant4_class == "G4UnionSolid" || geant4_class == "G4IntersectionSolid") {
    if (geant4_class == "G4IntersectionSolid" && HasNode(shape, "overlap_box_mm")) {
      ...
      const G4ThreeVector overlap_size = RequireVector3(YamlNode::Mapping(overlap), "size", context);
      return std::make_unique<G4Box>(
          name, 0.5 * overlap_size.x(), 0.5 * overlap_size.y(), 0.5 * overlap_size.z());
    }

    const auto& boxes = RequireNode(shape, "component_boxes_mm", context).AsSequence();
    if (boxes.size() != 2U) {
      throw std::runtime_error(context + ": boolean fixture requires exactly two component boxes");
    }
    ...
    auto* left_box = new G4Box(name + "_left", 0.5 * left_size.x(), 0.5 * left_size.y(), 0.5 * left_size.z());
    auto* right_box = new G4Box(name + "_right", 0.5 * right_size.x(), 0.5 * right_size.y(), 0.5 * right_size.z());
  ...
  if (geant4_class == "G4SubtractionSolid") {
    ...
    const std::vector<double> removed_min = RequireDoubleList(shape, "removed_box_min_mm", context);
    const std::vector<double> removed_size = RequireDoubleList(shape, "removed_box_size_mm", context);
    ...
    auto* removed_box = new G4Box(name + "_removed", 0.5 * removed_size[0], 0.5 * removed_size[1], 0.5 * removed_size[2]);
    const G4ThreeVector outer_center(0.5 * outer[0], 0.5 * outer[1], 0.5 * outer[2]);
    const G4ThreeVector removed_center(
        removed_min[0] + 0.5 * removed_size[0],
        removed_min[1] + 0.5 * removed_size[1],
        removed_min[2] + 0.5 * removed_size[2]);
    return std::make_unique<G4SubtractionSolid>(name, outer_box, removed_box, nullptr, removed_center - outer_center);
  }

  throw std::runtime_error("Unsupported boolean class in " + context + ": " + geant4_class);
}

std::unique_ptr<G4VSolid> BuildMultiUnionSolid(
    const std::string& name,
    const YamlNode& shape,
    const std::string& context) {
  ...
  for (std::size_t index = 0; index < boxes.size(); ++index) {
    ...
    auto* box = new G4Box(
        name + "_box_" + std::to_string(index), 0.5 * size.x(), 0.5 * size.y(), 0.5 * size.z());
...
std::unique_ptr<G4VSolid> BuildTessellatedSolid(
    const std::string& name,
    const YamlNode& shape,
    const std::string& context) {
  ...
  for (std::size_t facet_index = 0; facet_index < facets.size(); ++facet_index) {
    ...
    if (indices.size() != 3U) {
      throw std::runtime_error(context + ": triangular_facets entries must contain exactly 3 indices");
    }
...
std::unique_ptr<G4VSolid> BuildVTwistedFacetedFallback(
    const std::string& name,
    const YamlNode& ctor,
    const std::string& context) {
  ...
  for (const auto& point : std::array<G4TwoVector, 4>{
           G4TwoVector(-dx3 - top_shear, -dy2),
           G4TwoVector(dx3 - top_shear, -dy2),
           G4TwoVector(dx4 + top_shear, dy2),
           G4TwoVector(-dx4 + top_shear, dy2)}) {
    const G4TwoVector rotated = rotate(point.x(), point.y(), 0.5 * phi_twist);
...
std::unique_ptr<G4VSolid> BuildExtrudedSolid(
    const std::string& name,
    const YamlNode& shape,
    const std::string& context) {
  const std::vector<G4TwoVector> polygon = RequirePlanarPointList(shape, "polygon_vertices_mm", context);
  ...
  for (std::size_t index = 0; index < sections.size(); ++index) {
    ...
    const std::vector<double> offset = RequireDoubleList(YamlNode::Mapping(section), "offset", context);
...
std::unique_ptr<G4VSolid> BuildNativeSolid(const FixtureProvenance& provenance) {
  ...
  if (geant4_class == "G4Box") {
    ...
    return std::make_unique<G4Box>(name, 0.5 * dimensions[0], 0.5 * dimensions[1], 0.5 * dimensions[2]);
  }
  if (geant4_class == "G4Cons") {
    return std::make_unique<G4Cons>(name,
                                    RequireDouble(shape, "bottom_inner_radius_mm", context),
                                    ...
                                    0.5 * RequireDouble(shape, "height_mm", context),
                                    0.0,
                                    RequireDouble(shape, "delta_phi_deg", context) * deg);
  }
  if (geant4_class == "G4CutTubs") {
    return std::make_unique<G4CutTubs>(name,
                                       RequireDouble(ctor, "inner_radius_mm", context),
  ...
  if (geant4_class == "G4Para") {
    return std::make_unique<G4Para>(name,
                                    RequireDouble(shape, "half_x_mm", context),
  ...
  if (geant4_class == "G4Torus") {
    return std::make_unique<G4Torus>(name,
                                     RequireDouble(shape, "inner_tube_radius_mm", context),
  ...
  if (geant4_class == "G4Trap") {
    return std::make_unique<G4Trap>(name,
                                    RequireDouble(shape, "half_z_mm", context),
                                    RequireDouble(shape, "theta_deg", context) * deg,
                                    RequireDouble(shape, "phi_deg", context) * deg,
  ...
  if (geant4_class == "G4Trd") {
    return std::make_unique<G4Trd>(name,
                                   RequireDouble(shape, "bottom_half_x_mm", context),
  ...
  if (geant4_class == "G4Tubs") {
    return std::make_unique<G4Tubs>(name,
                                    RequireDouble(shape, "inner_radius_mm", context),
  ...
  if (geant4_class == "G4Ellipsoid") {
    return std::make_unique<G4Ellipsoid>(name,
                                         RequireDouble(ctor, "pxSemiAxis_mm", context),
  ...
  if (geant4_class == "G4EllipticalCone") {
    return std::make_unique<G4EllipticalCone>(name,
                                              RequireDouble(ctor, "pxSemiAxis", context),
  ...
  if (geant4_class == "G4EllipticalTube") {
    return std::make_unique<G4EllipticalTube>(name,
  ...
  if (geant4_class == "G4Hype") {
    return std::make_unique<G4Hype>(name,
                                    RequireDouble(ctor, "innerRadius_mm", context),
  ...
  if (geant4_class == "G4Paraboloid") {
    return std::make_unique<G4Paraboloid>(name,
                                          RequireDouble(ctor, "dz_mm", context),
  ...
  if (geant4_class == "G4Polycone") {
    ...
    return std::make_unique<G4Polycone>(name,
                                        RequireDouble(ctor, "phiStart_deg", context) * deg,
                                        RequireDouble(ctor, "phiTotal_deg", context) * deg,
  ...
  if (geant4_class == "G4Polyhedra") {
    ...
    const std::vector<double> r_inner = RequireDoubleList(ctor, "rInner_mm", context);
    const std::vector<double> r_outer = RequireDoubleList(ctor, "rOuter_mm", context);
    return std::make_unique<G4Polyhedra>(name,
                                         RequireDouble(ctor, "phiStart_deg", context) * deg,
                                         RequireDouble(ctor, "phiTotal_deg", context) * deg,
  ...
  if (geant4_class == "G4Tet") {
    const std::vector<G4ThreeVector> vertices = RequirePointList(shape, "vertices_mm", context);
    return std::make_unique<G4Tet>(name, vertices.at(0), vertices.at(1), vertices.at(2), vertices.at(3));
  }
  ...
  if (geant4_class == "G4TwistedBox") {
    return std::make_unique<G4TwistedBox>(name,
  ...
  if (geant4_class == "G4TwistedTrap") {
    return std::make_unique<G4TwistedTrap>(name,
                                           RequireDouble(ctor, "phi_twist_deg", context) * deg,
                                           RequireDouble(ctor, "dx1_mm", context),
                                           RequireDouble(ctor, "dx2_mm", context),
                                           RequireDouble(ctor, "dy_mm", context),
                                           RequireDouble(ctor, "dz_mm", context));
  }
  if (geant4_class == "G4TwistedTrd") {
    return std::make_unique<G4TwistedTrd>(name,
                                          RequireDouble(ctor, "dx1_mm", context),
                                          RequireDouble(ctor, "dx2_mm", context),
                                          RequireDouble(ctor, "dy1_mm", context),
                                          RequireDouble(ctor, "dy2_mm", context),
                                          RequireDouble(ctor, "dz_mm", context),
                                          RequireDouble(ctor, "phi_twist_deg", context) * deg);
  }
  ...
  if (geant4_class == "G4DisplacedSolid") {
    ...
    return std::make_unique<G4DisplacedSolid>(name, base_box, nullptr,
                                              G4ThreeVector(translation[0], translation[1], translation[2]));
  }
  if (geant4_class == "G4ScaledSolid") {
    ...
    return std::make_unique<G4ScaledSolid>(name, base_sphere, G4Scale3D(factors[0], factors[1], factors[2]));
  }
  if (geant4_class == "G4UnionSolid" || geant4_class == "G4IntersectionSolid" ||
...
G4ThreeVector FixtureComparisonOrigin(
  ...
  if (geant4_class == "G4Tet") {
    const std::vector<G4ThreeVector> vertices = RequirePointList(shape, "vertices_mm", provenance.source_path.string());
  ...
  const auto& generator = RequireNode(provenance.document, "generator", provenance.source_path.string()).AsMapping();
...
std::vector<G4ThreeVector> GenerateDirections(const std::size_t count) {
  ...
  for (std::size_t index = directions.size(); index < count; ++index) {
    ...
    const double z = 1.0 - 2.0 * k / static_cast<double>(std::max<std::size_t>(1, count - cardinal.size()));
...
RaySample TraceRay(const G4VSolid& solid, const G4ThreeVector& origin, const EInside state, const G4ThreeVector& direction) {
  RaySample sample;
...
ValidationReport CompareFixtureRays(
  ...
  if (options.ray_count == 0U) {
    report.AddError("fixture.ray_compare_invalid_count", "Ray count must be positive", request.manifest.source_path);
    return report;
  }
  ...
  try {
    ...
    for (const auto& direction : directions) {
      native_samples.push_back(TraceRay(*native_solid, local_summary.native_origin, local_summary.native_origin_state, direction));
    }
    ...
    for (const auto& direction : directions) {
      imported_samples.push_back(TraceRay(*imported_solid, local_summary.imported_origin, local_summary.imported_origin_state, direction));
    }
      ...
      if (native_sample.intersects != imported_sample.intersects) {
        ...
        if (local_summary.mismatch_count <= options.max_reported_mismatches) {
          report.AddError(
              ...
              "Ray " + std::to_string(index) + " hit mismatch for fixture '" + request.fixture.id +
                  "': native=" + (native_sample.intersects ? "hit" : "miss") + ", imported=" +
                  (imported_sample.intersects ? "hit" : "miss") + ", direction=" + ToString(directions[index]),
              provenance_path);
        }
        continue;
      }
    ...
    timing_summary << "Compared " << local_summary.ray_count << " rays for fixture '" << request.fixture.id
                   << "' (" << local_summary.geant4_class << ") with tolerance "
                   << local_summary.distance_tolerance << " mm; native="
                   << local_summary.native_elapsed_ms << " ms, imported="
                   << local_summary.imported_elapsed_ms << " ms, mismatches="
                   << local_summary.mismatch_count;
    report.AddInfo("fixture.ray_compare_summary", timing_summary.str(), provenance_path);
  } catch (const std::exception& error) {
    report.AddError(
        "fixture.ray_compare_failed",
