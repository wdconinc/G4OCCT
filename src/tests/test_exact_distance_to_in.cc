...
using g4occt::tests::navigation::ExpectExactDistanceToIn;
using g4occt::tests::navigation::SphereFixture;

TEST(ExactDistanceToIn, ShortestDistanceForExternalPoints) {
  const BoxFixture box("ExactDistanceToInBox", 10.0 * mm, 20.0 * mm, 30.0 * mm);
  ExpectExactDistanceToIn("box axis-aligned outside point uses face distance", box.solid,
  ...
  const SphereFixture sphere("ExactDistanceToInSphere", 50.0 * mm);
  ExpectExactDistanceToIn("sphere outside point uses radial distance", sphere.solid,
                          sphere.OutsideX(9.0 * mm), 9.0 * mm);

  const CylinderFixture cylinder("ExactDistanceToInCylinder", 25.0 * mm, 40.0 * mm);
  ExpectExactDistanceToIn("cylinder outside radial point uses radial distance", cylinder.solid,
                          cylinder.OutsideRadialX(6.0 * mm), 6.0 * mm);
}

TEST(ExactDistanceToIn, NearSurfaceBehaviour) {
  const BoxFixture box("ExactDistanceToInNearSurface", 10.0 * mm, 20.0 * mm, 30.0 * mm);
  const G4double surfaceTolerance = G4GeometryTolerance::GetInstance()->GetSurfaceTolerance();

  ExpectExactDistanceToIn("point exactly on the surface has zero safety", box.solid,
                          box.PositiveXSurface(), 0.0);

  const G4ThreeVector withinSurfaceShell(box.halfX + 0.25 * surfaceTolerance, 0.0, 0.0);
  ExpectExactDistanceToIn("point within the Geant4 surface shell has zero safety", box.solid,
  ...
  ExpectExactDistanceToIn("point just outside the surface shell returns a finite distance",
